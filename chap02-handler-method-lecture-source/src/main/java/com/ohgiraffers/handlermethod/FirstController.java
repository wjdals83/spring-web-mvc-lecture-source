...
@SessionAttributes(/* names = {"id", "pwd"} */"id")    // *두 개 이상일 때* "id"라는 이름의 키 값에 해당 파라미터 값을 넣어 둠.
public class FirstController {
    ...
    *   API 로 Servlet 에 종속(영향을 받지)적이지 않다.
    *   Spring 일부이기 떄문에 Servlet 을 사용하는 것처럼 동일하게
    *   사용할 수 있다.
    *  */
    @PostMapping("regist")
    public String registMenu(Model model, WebRequest request) {
        ...
        int price = Integer.parseInt(request.getParameter("price"));      // 웹에서 넘어오는 애들은 대부분 String 값이라 파싱
    ...
    *  */
    ...
    @PostMapping("search")
    public String searchMenu(@ModelAttribute MenuDTO menu) {        // @ModelAttribute 는 생략이 가능하지만 명시적으로 작성하는 것이 좋다.
    ...
    public String logoutTest1(HttpSession session) {
        ...
        session.invalidate();       // invalidate 강제 만료시키는 애
    ...
    *  */
    public void bodyTest(@RequestBody String body, @RequestHeader("content-type") String contentType) throws UnsupportedEncodingException {
        ...
        System.out.println(URLDecoder.decode(body, "UTF-8"));   // decode 는 예외처리
    }
