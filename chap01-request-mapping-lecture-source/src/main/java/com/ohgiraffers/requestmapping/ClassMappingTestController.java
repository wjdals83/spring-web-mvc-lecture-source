...
*   내용을 작성하지 않아도 된다.
*   이 떄 와일드카드(*) 를 이용해서 포괄적인 URL 패턴을 이용할 수 있다.
*  */
public class ClassMappingTestController {

    @GetMapping("/regist")      // 자연스럽게 order 하위에 regist URL 이 붙게 된다.
    ...
    *   path variable 로 전달 되는 { 변수명 } 은 반드시ㅣ 매개변수 명과 동일해야 한다.
    ...
    *  */
    ...
    // orderNo 라는 경로 변수를 받는다. 이 경로 변수는 @PathVariable 어노테이션을 사용하여 메소드 매개변수로 매핑된다.
        model.addAttribute("message",
                orderNo2 + "번 주문 상세 내용 조회용 핸들러 메소드 호출됨...");

        return "mappingResult"; //mappingResult 이름의 뷰 반환
    }
